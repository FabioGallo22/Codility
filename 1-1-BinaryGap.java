...
PROBLEM DESCRIPTION (Source: https://app.codility.com/programmers/lessons/1-iterations/binary_gap/)
...
class Solution {
    public int solution(int N) {
        ...
        for (int i=0; i < binaryNum.length(); i++){
            String digit = binaryNum.substring(i,i+1);
            if (digit.compareTo("1") == 0){
                if(findOne){
                    // Already found a sequence between two 1s
                    if (cant > maxSeq){
                        maxSeq = cant;
                    }
                    cant = 0; 
                ...
                // only cant is count is previusly there was an 1
                if (findOne){
                    cant ++;
                }
            }
