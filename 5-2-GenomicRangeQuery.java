...
PROBLEM DESCRIPTION (Source: https://app.codility.com/programmers/lessons/5-prefix_sums/genomic_range_query/)
	...
	several queries of the form: What is the minimal impact factor of nucleotides contained in a particular 
...
class Solution {
    public int[] solution(String S, int[] P, int[] Q) {
        int[] arrayImp = generateImpactArray(S);
        int[] result = new int[P.length];
      
        for(int indexQ = 0; indexQ < P.length; indexQ++){
            int minImpact = 5;
            for(int i = P[indexQ]; i <= Q[indexQ]; i++){
                if(arrayImp[i] < minImpact){
                    minImpact = arrayImp[i];
                }
            }
            result[indexQ] = minImpact;
        }
        return result;
    }
    
    public int[] generateImpactArray(String s){
